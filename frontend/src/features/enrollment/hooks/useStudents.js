...
export function useCreateStudent() {
    ...
    return useMutation({
        ...
        onMutate: async (newStudent) => {
            ...
            queryClient.setQueriesData({ queryKey: queryKeys.students.lists() }, (old) => {
                if (!old)
                    return old;
...
export function useUpdateStudent(id) {
    ...
    return useMutation({
        ...
        onMutate: async (updatedStudent) => {
            ...
            queryClient.setQueryData(queryKeys.students.detail(id), (old) => {
                if (!old)
                    return old;
            ...
            queryClient.setQueriesData({ queryKey: queryKeys.students.lists() }, (old) => {
                if (!old)
                    return old;
                return old.map((student) => student.id === id ? { ...student, ...updatedStudent } : student);
            });
...
export function useEnrollStudent() {
    ...
    return useMutation({
        ...
        onMutate: async (enrollmentData) => {
            ...
            const previousEnrollments = queryClient.getQueryData(queryKeys.students.enrollments(enrollmentData.student_id));
            // Optimistically add the new enrollment
            queryClient.setQueryData(queryKeys.students.enrollments(enrollmentData.student_id), (old) => {
                if (!old)
                    return old;
        ...
        onError: (err, enrollmentData, context) => {
            ...
            if (context?.previousEnrollments) {
                queryClient.setQueryData(queryKeys.students.enrollments(enrollmentData.student_id), context.previousEnrollments);
            }
        },
