...
export interface ApiError extends Error {
  ...
  category: 'validation' | 'network' | 'server' | 'auth' | 'not_found' | 'conflict';
}
...
function getSpecificErrorMessage(status: number, responseText: string, path: string): string {
  ...
  switch (status) {
    ...
    case 404:
      if (path.includes('/students/')) return 'Student not found.';
      if (path.includes('/classrooms/')) return 'Classroom not found.';
      if (path.includes('/academic-years/')) return 'Academic year not found.';
